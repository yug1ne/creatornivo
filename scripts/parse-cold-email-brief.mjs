/**
 * One-off: parse cold-email brief from session prompt into schema JSON.
 * Usage: node scripts/parse-cold-email-brief.mjs [path-to-brief.txt]
 */
...
const briefPath =
  ...
  path.join(
    ...
    ".grok/sessions/F%3A%5Ccreatornivo/019f505d-6680-70f1-adb8-cb5fadaff5be/prompts/prompt_9.txt",
  );
...
function grabOptions(block) {
  ...
  for (const line of block.slice(i).split("\n")) {
    ...
    if (t.startsWith("* ")) opts.push(t.slice(2).trim());
  }
  return opts;
}
...
const out = path.join(root, "agent-tools", "cold-email-schema-parsed.json");
