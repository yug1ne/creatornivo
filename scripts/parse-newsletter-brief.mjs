/**
 * Parse Newsletter Standard brief into schema JSON.
 * Usage: node scripts/parse-newsletter-brief.mjs [path-to-brief.txt]
 */
...
const briefPath =
  ...
  path.join(
    ...
    ".grok/sessions/F%3A%5Ccreatornivo/019f505d-6680-70f1-adb8-cb5fadaff5be/prompts/prompt_18.txt",
  );
...
function extractFieldBlock(key) {
  ...
  if (idx < 0) {
    // bare Field key: style without backticks
    const bare = "Field key:\n" + key;
    idx = p.indexOf(bare);
    if (idx < 0) {
...
const fields = keys.map((key) => {
  ...
  const typeRaw = grab(block, "Type") || "text";
  const requiredRaw = grab(block, "Required");
  const defaultRaw = grab(block, "Default") || "Blank";
...
const out = path.join(root, "agent-tools", "newsletter-schema-parsed.json");
