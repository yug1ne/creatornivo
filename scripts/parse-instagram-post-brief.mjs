/**
 * Parse Instagram Post brief into schema JSON.
 * Usage: node scripts/parse-instagram-post-brief.mjs [path-to-brief.txt]
 */
...
const briefPath =
  ...
  path.join(
    ...
    ".grok/sessions/F%3A%5Ccreatornivo/019f505d-6680-70f1-adb8-cb5fadaff5be/prompts/prompt_13.txt",
  );
...
function grabOptions(block) {
  ...
  for (const line of block.slice(i).split("\n")) {
    ...
    // numbered list: "1. Build awareness" or "* item"
    const num = t.match(/^\d+\.\s+(.+)/);
    if (num) {
      opts.push(num[1].trim());
      continue;
    }
    if (t.startsWith("* ")) {
...
const out = path.join(root, "agent-tools", "instagram-post-schema-parsed.json");
