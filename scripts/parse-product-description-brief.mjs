/**
 * Parse Product Description Standard brief into schema JSON.
 * Usage: node scripts/parse-product-description-brief.mjs [path-to-brief.txt]
 */
...
const briefPath =
  ...
  path.join(
    ...
    ".grok/sessions/F%3A%5Ccreatornivo/019f505d-6680-70f1-adb8-cb5fadaff5be/prompts/prompt_20.txt",
  );
...
function extractFieldBlock(key) {
  ...
  const start = Math.max(p.lastIndexOf("### Field", idx), idx - 50);
  const nextField = p.indexOf("### Field", idx + 1);
  const nextKey = p.indexOf("\nField key:", idx + needle.length);
